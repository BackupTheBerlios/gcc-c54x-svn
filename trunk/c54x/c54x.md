...
(define_predicate "mmr_register_operand"
  (and (match_code "reg")
	   (match_test "MMR_REG_P(op)||PSEUDO_REG_P(op)")))

(define_predicate "arsp_register_operand"
  (and (match_code "reg")
	   (match_test "AUX_REG_P(op)||SP_REG_P(op)||ARG_REG_P(op)||PSEUDO_REG_P(op)")))

(define_predicate "adddst_operand"
  (and (match_code "reg,mem")
	   (match_test "ACC_REG_P(op)||MEM_P(op)||PSEUDO_REG_P(op)")))

(define_predicate "addsrc_operand"
  (and (match_code "mem,const,const_int,reg")
	   (match_test "MEM_P(op)||CONSTANT_P(op)||ACC_REG_P(op)||PSEUDO_REG_P(op)")))
...
(define_insn "sth"
  [(set (match_operand:QI 0 "nonimmediate_operand" "=T")
		(truncate:QI (ashift:ACC (match_operand:ACC 1 "register_operand" "c")
								 (const_int -16))))]
""
